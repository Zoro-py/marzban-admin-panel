...
export function CustomersPage() {
  ...
  const filtered = React.useMemo(() => {
    ...
    const rows = familiesOnly ? data.filter((c) => c.kind === 'family') : data
    if (!q) return rows
    return rows.filter((c) => c.name.toLowerCase().includes(q) || c.contact?.toLowerCase().includes(q))
  }, [data, search, familiesOnly])
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button
          type="button"
          size="sm"
          variant={familiesOnly ? 'default' : 'outline'}
          aria-pressed={familiesOnly}
          onClick={() => setFamiliesOnly((v) => !v)}
        >
          Families only
        </Button>
      </div>
            ...
            {filtered.map((c) => (
              <TableRow key={c.id} className="cursor-pointer" onClick={() => navigate(`/customers/${c.id}`)}>
                <TableCell className="font-medium">
                  {c.name}
                  {c.kind === 'family' && <Badge variant="secondary" className="ml-2">Family</Badge>}
                </TableCell>
